...
int FdkAacEncoder::alloc_audio_stream(const char * codec_name) {
    AVCodec *codec;
...
int FdkAacEncoder::alloc_avframe() {
    int ret = 0;
    AVSampleFormat preferedSampleFMT = AV_SAMPLE_FMT_S16;
    int preferedChannels = audioChannels;
    int preferedSampleRate = audioSampleRate;
...
int FdkAacEncoder::init(int bitRate, int channels, int sampleRate, int bitsPerSample, const char* aacFilePath, const char * codec_name) {
    avCodecContext = NULL;
...
int FdkAacEncoder::init(int bitRate, int channels, int bitsPerSample, const char* aacFilePath, const char * codec_name) {
    return init(bitRate, channels, 44100, bitsPerSample, aacFilePath, codec_name);
}

void FdkAacEncoder::encode(char* buffer, int size) {
    int bufferCursor = 0;
    int bufferSize = size;
...
void FdkAacEncoder::encodePacket() {
